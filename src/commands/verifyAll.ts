...
export async function cmdVerifyAll(): Promise<number> {
  ...
  for (const r of rotations) {
    ...
    if (!r.destinations.includes("onepassword")) {
      log.warn(`${label} ${c.dim("skipped — no 1Password destination")}`); skipped++; continue;
    }
    ...
    if (provider.id === "generic") {
      log.warn(`${label} ${c.dim("skipped — generic provider has no verifier")}`); skipped++; continue;
    }
    ...
    if (vRes.ok) { log.ok(`${label} ${c.green("OK")}   ${c.dim(provider.label + " — " + vRes.detail)}`); passed++; }
    else { log.err(`${label} ${c.red("FAIL")} ${c.dim(provider.label + " — " + vRes.detail)}`); failed++; }
  }
  log.blank();
